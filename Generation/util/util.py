...
def write_file(args, folder, patch, file_name, bug, skip_val=True, lang="python", reset=False):
    ...
    message = "Not needed"
    if lang == "python":
        print(f"Validating Python patch for bug {bug}...")
        exit_code = subprocess.run(
            f"cd ../QuixBugs; python python_tester.py --bug {bug} --file {file_path} --add_pf",
            shell=True,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL
        )
        valid = exit_code.returncode == 0
        error_file_path = os.path.join("../QuixBugs", "error.txt")
